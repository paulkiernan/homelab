...
The cluster capacity rules are neither vendored nor taken from
diff-logic-cells: alerts/homelab-capacity.yaml is hand-authored source in this
repository (see the file header for the gaps it closes that upstream leaves
open). It is a Prometheus rule file, so it goes through the same converter and
the same validations as the upstream alert rules, and its rules land in the
same Grafana folder. Nothing under alerts/ is generated; this script only reads
it.
...
Outputs (all generated, all committed; run with --check to detect drift):
  ...
  manifests/grafana-alerting-homelab-configmap.yaml
...
# Hand-authored rules owned by this repository: alerts/ is source, not
# generated. They are converted exactly like the vendored upstream alert rules,
# so every cluster alert shares one folder, one converter and one delivery path.
HOMELAB_CAPACITY_SOURCE = HERE / "alerts" / "homelab-capacity.yaml"
HOMELAB_ALERTING_CONFIGMAP = "local-monitoring-grafana-alerting-homelab"
HOMELAB_RULES_KEY = "homelab-capacity-rules.yaml"
HOMELAB_GENERATED_HEADER = (
    "# Generated by generate-assets.py from the hand-authored "
    "alerts/homelab-capacity.yaml — do not edit by hand.\n"
)
# The source file holds exactly this many alert rules; adding or removing one
# means updating the count in the same change, so no rule can go missing
# unnoticed.
EXPECTED_HOMELAB_ALERT_RULES = 9
...
# Job values this cluster's own Prometheus scrapes. These are real local
# targets, not kube-prometheus leftovers, so the residue check accepts them by
# exact value rather than by widening its pattern: the hand-authored
# HomelabPrometheusStorageHigh reads the server's own self-scrape job, which
# Alloy does not produce and which therefore has no `integrations/` prefix.
# Every other non-integration job value (kubelet, node-exporter, apiserver, …)
# still dies.
LOCAL_JOB_VALUES = frozenset({"prometheus"})
...
def assert_selectors_rewritten(origin: str, document: object) -> None:
    ...
    Every job selector must name an Alloy remote-write job (integrations/...), a
    job this cluster scrapes locally (LOCAL_JOB_VALUES), or a dashboard template
    variable ($ksm_job, ...; a bare regex wildcard carries no literal target
    either). `metrics_path` is not a label on remote-written series at all, so
    any occurrence is residue of the kube-prometheus scrape config. Emitting one
    of those would provision panels and rules that match nothing while looking
    healthy.
    """
    for text in strings(document):
        ...
        for value in JOB_SELECTOR.findall(text):
            if (
                value.startswith((INTEGRATION_JOB_PREFIX, "$"))
                or value in LOCAL_JOB_VALUES
                or not re.search(r"[A-Za-z0-9_/]", value)
            ):
                continue
...
def convert_alert_rules(pairs: list[tuple[str, dict]]) -> list[dict]:
    """Convert a set of Prometheus alerting rules to Grafana rules, in order.

    The uid and title scheme needs the whole set, not one rule at a time: an
    alertname shipped once becomes uid == title == alertname, while a name
    shipped as a warning/critical pair gets the severity appended to both,
    because Grafana keys rules by uid and requires unique titles within a
    folder. Both the vendored upstream set and the hand-authored homelab set go
    through this one function so the conversion and its validations can never
    drift apart.
    """
    counts: dict[str, int] = {}
    for _, rule in pairs:
        counts[rule["alert"]] = counts.get(rule["alert"], 0) + 1

    converted: list[dict] = []
    for context, rule in pairs:
        rendered = convert_alert_rule(rule, context, unique_name=counts[rule["alert"]] == 1)
        validate_rule(context, rendered)
...
def build_kubernetes_alert_groups(groups: dict[str, dict]) -> list[dict]:
    ...
    converted = convert_alert_rules([(f"upstream group {name!r}", rule) for name, rule in pairs])
    grouped: dict[str, list[dict]] = {name: [] for name in ALERT_GROUPS}
    for (name, _), rendered in zip(pairs, converted):
        grouped[name].append(rendered)
...
def build_homelab_alert_group(path: Path) -> dict:
    """The hand-authored homelab capacity rules as one Grafana group.

    The source is a Prometheus rule file (a top-level `groups:` list, the shape
    of a PrometheusRule's spec), so the rules stay loadable by Prometheus
    unchanged and convert through the same path as the vendored upstream rules.
    Only the shape this generator needs is enforced here; whether a given rule
    is worth having is the file's own business, and its header explains why each
    one exists.
    """
    document = yaml.safe_load(path.read_text(encoding="utf-8"))
    groups = (document or {}).get("groups")
    if not isinstance(groups, list) or len(groups) != 1:
        die(f"{path} must hold exactly one rule group under `groups:`, found {groups!r}")
    group = groups[0]
    name = group.get("name")
    if not name:
        die(f"{path}: the rule group needs a name (it becomes the Grafana group name)")
    rules = group.get("rules") or []
    if not rules:
        die(f"{path}: group {name!r} contains no rules")
    for rule in rules:
        if not rule.get("alert"):
            die(f"{path}: group {name!r} carries a rule without an `alert` name: {rule!r}")
        if "record" in rule:
            die(f"{path}: alert {rule['alert']!r} also carries a `record` name; split it out")
        if not rule.get("expr"):
            die(f"{path}: alert {rule['alert']!r} has no `expr`")

    converted = convert_alert_rules([(f"{path} group {name!r}", rule) for rule in rules])
    if len(converted) != EXPECTED_HOMELAB_ALERT_RULES:
        die(
            f"{path} holds {len(converted)} alert rules, expected {EXPECTED_HOMELAB_ALERT_RULES} "
            "(update the count in the same change)"
        )
    return {
        "orgId": 1,
        "name": name,
...
def build() -> dict[Path, str]:
    ...
    homelab_group = build_homelab_alert_group(HOMELAB_CAPACITY_SOURCE)
    ...
    # is fatal for the whole process. Fail loudly instead. The hand-authored
    # homelab rules are checked in the same sets as the generated ones: they
    # share the Kubernetes folder, so their names, uids and titles have to be
    # unique against the vendored set, and their uids against the market rules.
    all_groups = [*groups, *kube_groups, homelab_group]
    ...
    kube_titles = [
        rule["title"] for group in (*kube_groups, homelab_group) for rule in group["rules"]
    ]
    ...
    homelab_alerting_configmap = {
        "apiVersion": "v1",
        "kind": "ConfigMap",
        "metadata": {
            "name": HOMELAB_ALERTING_CONFIGMAP,
            "labels": {ALERTS_SIDECAR_LABEL: "1"},
        },
        "data": {HOMELAB_RULES_KEY: yaml_block({"apiVersion": 1, "groups": [homelab_group]})},
    }
    ...
    files[MANIFESTS / "grafana-alerting-homelab-configmap.yaml"] = (
        HOMELAB_GENERATED_HEADER + yaml_block(homelab_alerting_configmap)
    )
