...
- `task measurement:vm-install` then ships the binary and systemd units over SSH in one checksummed payload. It requires `MSR_VM_TARGET`, `MSR_VM_SITE` and `MSR_VM_BINARY` — a real locally built Linux binary, whose ELF architecture the wrapper verifies — plus a pinned host key: `MSR_VM_KNOWN_HOSTS` (preferred) or `MSR_VM_FINGERPRINT`; `MSR_VM_TOFU=1` is the last resort. `MSR_VM_PULL_KEY` authorizes the read-only rsync pull key on the host. The approved instance is deployed: Lightsail `market-measurement` in `us-east-1a` (`micro_3_0`, the ~$7/month class, static IPv4 `107.21.239.171`, site `aws-us-east-1`); OpenTofu state stays local and git-ignored under `terraform/market-measurement/`, and `user_data` is create-time-only (`lifecycle.ignore_changes = [user_data]`), so launch-script fixes change what future instances boot with while existing hosts are remediated through the phase-2 installer. The SSH-pull CronJob was suspended until the cloud side actually existed, so a homelab-only deployment scheduled no cloud pulls; it is active since 2026-09-17, after the VM was provisioned, the pull key was authorized on it, the SOPS-encrypted `ssh-pull-secret.sops.yaml` and its kustomization entry were committed, and one `task measurement:pull-run` Job had verified sealed chunks on the NAS — only then was `suspend` set to false in Git. Once enabled, a missing Secret or an unreachable VM keeps failing the pull Job visibly; suspension was a deliberate pending state, never a substitute for a working transport.
